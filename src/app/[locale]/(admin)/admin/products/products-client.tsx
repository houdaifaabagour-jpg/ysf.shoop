...
import { 
  Search, Filter, Plus, ChevronDown, ChevronUp, Trash2, 
  ImagePlus, X, Loader2, Star, Eye, EyeOff, Tag, AlertTriangle, Layers, ArrowUpDown
} from "lucide-react";
...
export function ProductsClient({ initialProducts, categories, currencySymbol, locale }: ProductsClientProps) {
  ...
  const handleCreateProduct = async (formData: FormData) => {
    // Append tag list as comma separated string
    ...
    // Replace images input files with our final deferred list
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (!confirm("Are you sure you want to permanently delete this product and all associated images?")) return;
    startTransition(async () => {
      try {
  ...
  const getProductStockStatus = (p: Product) => {
    ...
    if (p.variants && p.variants.length > 0) {
      if (totalStock === 0) return { label: "Out of Stock", color: "bg-red-50 text-red-700 border-red-200", value: "out" };
      if (totalStock < 10) return { label: `Low Stock (${totalStock})`, color: "bg-amber-50 text-amber-700 border-amber-200", value: "low" };
      return { label: `In Stock (${totalStock})`, color: "bg-emerald-50 text-emerald-700 border-emerald-200", value: "instock" };
    }
    return { label: "No variants", color: "bg-gray-50 text-gray-500 border-gray-200", value: "none" };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-text-muted mt-1">Manage your luxury watch and glasses catalog, inventory, and images.</p>
        </div>
        <button
          onClick={() => setIsFormOpen(!isFormOpen)}
          className="btn-luxury btn-luxury-primary self-start sm:self-auto flex items-center justify-center gap-2"
        >
        ...
        <div className="rounded-2xl border border-neutral-border bg-white shadow-lg overflow-hidden transition-all duration-300">
          <div className="bg-neutral-warm/50 px-6 py-4 border-b border-neutral-border-light flex justify-between items-center">
            <h2 className="text-base font-bold text-primary flex items-center gap-2">
              <Plus className="w-4 h-4 text-accent" /> Add New Boutique Product
            </h2>
            <button onClick={() => setIsFormOpen(false)} className="text-neutral-text-muted hover:text-primary transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <form action={handleCreateProduct} className="p-6 space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-4">
                <h3 className="text-xs font-bold uppercase tracking-wider text-accent border-b border-neutral-border-light pb-1">Product Details</h3>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="col-span-full">
                    ...
                    <input 
                      ...
                      required 
                      className="w-full rounded-lg border border-neutral-border-light px-4 py-2.5 text-sm focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent" 
                    />
                  </div>
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-accent border-b border-neutral-border-light pb-1 mb-4">Product Images (Deferred Upload)</h3>
                  
                  {/* Dropzone withPreviews */}
                  <div 
                  ...
                  >
                    ...
                    <p className="text-xs font-bold text-primary">Drop product photos or click to browse</p>
                    <p className="mt-1 text-[10px] text-neutral-text-muted">Recommended: Square 1:1 Aspect Ratio • JPEG/PNG/WebP • Max 5MB</p>
                    <input 
                      ref={fileInputRef}
                      type="file" 
                      name="images" 
                      ...
                      {previewUrls.map((url, i) => (
                        <div key={url} className="group relative aspect-square rounded-lg overflow-hidden border border-neutral-border bg-neutral-warm">
                          ...
                          <button 
                            ...
                            className="absolute top-1 right-1 rounded-full bg-white/80 p-1 text-red-600 shadow-sm opacity-100 sm:opacity-0 group-hover:opacity-100 hover:bg-red-600 hover:text-white transition-all"
                            title="Remove image"
                          >
                            <X className="w-3 h-3" />
                          </button>
                ...
                {/* Sku and Tags chip input */}
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-accent border-b border-neutral-border-light pb-1 mb-4">Metadata & Tags</h3>
                  <div>
                    <label className="block text-xs font-bold text-neutral-text-muted mb-1.5">Interactive Tags (Press Enter)</label>
                    <div className="flex flex-wrap gap-1.5 p-2 rounded-lg border border-neutral-border-light bg-neutral-warm/10 min-h-11">
                      ...
                      <input 
                        ...
                        placeholder={tags.length === 0 ? "e.g., rolex, daytona, gold" : "Add tag..."}
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-neutral-border-light">
              ...
              <button 
                ...
                className="text-sm font-semibold text-neutral-text-muted hover:text-primary transition-colors px-4 py-2"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="space-y-3">
              ...
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-neutral-text-muted font-medium">
                    ...
                    <span>&middot;</span>
                    <span>{sorted.length} photo{sorted.length !== 1 ? "s" : ""}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 pt-3 sm:pt-0 border-neutral-border-light shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    className="rounded-full border border-red-200 px-3 py-1.5 text-xs font-bold text-red-600 hover:bg-red-50 transition-colors"
                  >
        ...
        {filteredProducts.length === 0 && (
          <div className="rounded-xl border border-neutral-border bg-white py-12 text-center">
            ...
            <p className="text-xs text-neutral-text-muted mt-1">Try adjusting your filters or add a new luxury product.</p>
          </div>
        )}
      </div>
    </div>
