...
const links = [
  ...
  { href: "/admin/audit-logs", label: "Audit", icon: "M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
...
export function AdminSidebar({ locale = "ar" }: AdminSidebarProps) {
  return (
    <>
      <aside className="fixed right-0 top-0 z-50 hidden lg:block h-screen w-60 bg-warm border-l border-border p-5 overflow-y-auto">
        <Link href={`/${locale}/admin`} className="mb-8 block text-base font-bold tracking-tight text-primary">
          ysf.shoop
        </Link>
        <nav className="flex flex-col gap-0.5">
          {links.map((link) => (
            <Link
              ...
              className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-white transition-all"
            >
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-border bg-white px-2 py-2 lg:hidden">
        {links.slice(0, 5).map((link) => (
          <Link
            key={link.href}
            href={`/${locale}${link.href}`}
            className="flex flex-col items-center gap-0.5 px-2 py-1 text-[10px] font-medium text-muted-foreground hover:text-primary transition-colors"
          >
